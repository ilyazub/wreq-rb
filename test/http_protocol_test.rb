...
class HttpProtocolTest < Minitest::Test
  ...
  def test_httpbin_http2_support
    # httpbin.org supports HTTP/2; verify we can connect
  ...
  def test_concurrent_requests_to_same_host
    ...
    threads = Array.new(5) do |i|
      Thread.new do
        response = HTTP.get("https://httpbin.org/get?request=#{i}")
  ...
  def test_high_concurrency_many_threads
    ...
    threads = Array.new(10) do
      Thread.new do
        2.times do |i|
          response = HTTP.get("https://httpbin.org/get?t=#{Thread.current.object_id}&i=#{i}")
          responses << response.status.to_i
        end
      rescue StandardError => e
        errors << e
  ...
  def test_post_large_json_payload
    ...
    response = HTTP.post('https://httpbin.org/post', json: large_data)
    assert_equal 200, response.status.to_i
  ...
  def test_post_large_form_data
    ...
    response = HTTP.post('https://httpbin.org/post', form: form_data)
    assert_equal 200, response.status.to_i
  ...
  def test_timeout_on_slow_endpoint
    ...
    assert_raises(RuntimeError) do
      HTTP.timeout(0.5).get('https://httpbin.org/delay/5')
    end
  end
  ...
  def test_timeout_chainable
    ...
    response = HTTP.timeout(30).headers(accept: :json).get('https://httpbin.org/get')
    assert_equal 200, response.status.to_i
  end
  ...
  def test_follow_single_redirect
    ...
    response = HTTP.follow.get('https://httpbin.org/redirect/1')
    assert_equal 200, response.status.to_i
  ...
  def test_follow_multiple_redirects
    ...
    response = HTTP.follow(max_hops: 5).get('https://httpbin.org/redirect/3')
    assert_equal 200, response.status.to_i
    assert_equal 'https://httpbin.org/get', response.uri
  end
  ...
  def test_follow_default_true
    ...
    response = HTTP.follow.get('https://httpbin.org/redirect/2')
    assert_equal 200, response.status.to_i
  end
  ...
  def test_put_request_with_body
    ...
    response = HTTP.put('https://httpbin.org/put', body: 'updated data')
    assert_equal 200, response.status.to_i
  ...
  def test_patch_request_with_body
    ...
    response = HTTP.patch('https://httpbin.org/patch', body: 'patched data')
    assert_equal 200, response.status.to_i
  ...
  def test_delete_request_with_params
    ...
    response = HTTP.delete('https://httpbin.org/delete', params: { id: '123' })
    assert_equal 200, response.status.to_i
  ...
  def test_status_201_created
    # Test 201 Created (POST with json at httpbin)
    response = HTTP.post('https://httpbin.org/post', json: { test: 'data' })
    assert_equal 200, response.status.to_i # NOTE: httpbin returns 200, not 201
  end
  ...
  def test_status_304_not_modified
    ...
    response = HTTP.get('https://httpbin.org/cache')
    assert_equal 200, response.status.to_i
    # NOTE: httpbin/cache doesn't truly support caching, but endpoint exists
  end

  def test_status_400_bad_request
    # Test 400 Bad Request
    response = HTTP.get('https://httpbin.org/status/400')
  ...
  def test_status_403_forbidden
    # Test 403 Forbidden
    response = HTTP.get('https://httpbin.org/status/403')
    assert response.status.client_error?
  ...
  def test_response_with_custom_headers
    ...
    response = HTTP.headers('X-Custom-Header' => 'custom-value').get('https://httpbin.org/get')
    assert_equal 200, response.status.to_i
  ...
  def test_cookies_in_request
    ...
    response = HTTP.cookies(session: 'abc123', user: 'testuser').get('https://httpbin.org/cookies')
    assert_equal 200, response.status.to_i
  ...
  def test_accept_header_json
    ...
    response = HTTP.accept(:json).get('https://httpbin.org/get')
    assert_equal 200, response.status.to_i
  ...
  def test_accept_header_html
    ...
    response = HTTP.accept(:html).get('https://httpbin.org/html')
    assert_equal 200, response.status.to_i
  ...
  def test_get_with_query_params
    ...
    response = HTTP.get('https://httpbin.org/get', params: { search: 'test', page: '2' })
    assert_equal 200, response.status.to_i
  ...
  def test_post_with_json_body
    ...
    response = HTTP.post('https://httpbin.org/post', json: { name: 'Alice', age: 30 })
    assert_equal 200, response.status.to_i
  ...
  def test_post_with_form_body
    ...
    response = HTTP.post('https://httpbin.org/post', form: { name: 'Bob', email: 'bob@example.com' })
    assert_equal 200, response.status.to_i
  ...
  def test_post_with_raw_body
    ...
    response = HTTP.post('https://httpbin.org/post', body: 'raw data here')
    assert_equal 200, response.status.to_i
  ...
  def test_chainable_headers_and_timeout
    ...
    response = HTTP.headers(accept: :json).timeout(30).get('https://httpbin.org/get')
    assert_equal 200, response.status.to_i
  end
  ...
  def test_chainable_full_composition
    ...
    response = HTTP
               ...
               .accept(:json)
               .get('https://httpbin.org/get', params: { test: 'true' })
  ...
  def test_request_method_post
    ...
    response = HTTP.request(:post, 'https://httpbin.org/post', json: { test: 'data' })
    assert_equal 200, response.code
  end

  def test_request_method_put
    # Test request method with :put symbol
    response = HTTP.request(:put, 'https://httpbin.org/put', body: 'data')
    assert_equal 200, response.code
  end

  def test_request_method_delete
    # Test request method with :delete symbol
    response = HTTP.request(:delete, 'https://httpbin.org/delete')
    assert_equal 200, response.code
  end
  ...
  def test_request_method_invalid_raises_error
    ...
    assert_raises(ArgumentError) do
      HTTP.request(:invalid_method, 'https://httpbin.org/get')
    end
  end
  ...
  def test_api_workflow_get_and_transform
    ...
    response = HTTP.accept(:json).get('https://httpbin.org/get', params: { data: 'test' })
    assert_equal 200, response.status.to_i

    data = response.parse
    assert_equal 'test', data['args']['data']
    assert_includes data['url'], 'httpbin.org/get'
  end
